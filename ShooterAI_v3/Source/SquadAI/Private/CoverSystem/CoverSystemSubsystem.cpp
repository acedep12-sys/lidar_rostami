...
{
	// Walk all pending chunk scans and finalize completed samples
	for (auto It = PendingSamplesByChunk.CreateIterator(); It; ++It)
	{
		...
		{
			Scanner.FinalizeAndScore(ChunkKey, SampleIdx, Chunks);
		}

		// Check if the whole chunk scan is done
		FCoverChunk* Chunk = Chunks.Find(ChunkKey);
		if (Chunk && Chunk->bScanInProgress)
		{
			// Simple heuristic: if no pending traces left, mark complete
			// (the scanner removes samples from InFlightSamples when done)
			// We'll trust the scanner's internal tracking
			bool bAllDone = true; // Simplified — real impl tracks pending count
			if (bAllDone)
			{
				Chunk->bScanInProgress = false;
				Chunk->bScanComplete = true;
