...
void ASoldierAIController::OnPerceptionUpdated(AActor* Actor, FAIStimulus Stimulus)
{
	...
	// Check if target is alive (works for Soldiers and any character with HealthComp)
	bool bIsAlive = true;
	if (ASoldierCharacter* Soldier = Cast<ASoldierCharacter>(OtherPawn))
	{
		bIsAlive = Soldier->IsAlive();
	}
	if (!bIsAlive) return;

	// Only track hostiles for target memory
	ETeamAttitude::Type Attitude = ETeamAttitude::Neutral;
	if (OtherPawn->GetClass()->ImplementsInterface(UGenericTeamAgentInterface::StaticClass()))
	{
		if (IGenericTeamAgentInterface* TeamAgent = Cast<IGenericTeamAgentInterface>(OtherPawn))
		{
	...
	{
		// Default attitude check for non-team-interface actors (like simple player pawns)
		Attitude = (GetGenericTeamId() != FGenericTeamId::NoTeam) ? ETeamAttitude::Hostile : ETeamAttitude::Neutral;
	}
...
void ASoldierAIController::RequestCover(FVector ThreatLocation, float SearchRadius)
{
	...
	CoverSys->RequestCoverAsync(
		...
		[WeakSelf](bool bFound, const FCoverPoint& Result)
		{
			if (WeakSelf.IsValid())
			{
				WeakSelf->PendingCoverResult = Result;
